...
When the dialog opens and the grind wheel has no candidates to offer, it SHALL
open with the text fields already shown and focused rather than displaying an
empty or unusable wheel. This applies exactly when no grind rows can be
generated: either there is no current value and no observed history, or the
current value cannot be parsed by the stepper AND there is no observed history to
fall back on. An unparseable value WITH observed history SHALL stay on the
wheels — the history fallback handles it.
...
- **GIVEN** a recorded grind value the stepper cannot parse and no observed history
