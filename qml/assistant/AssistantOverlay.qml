...
Item {
    ...
    Timer {
        ...
        onTriggered: {
            ...
            root._trySpeakQuickFiller()   // speak a filler that already arrived while we were holding it
        }
    }
    ...
    function _trySpeakQuickFiller() {
        ...
        if (root._state !== "conversing" || !root._thinking || root._spokeThisTurn
            ...
            root._diag("quick_filler_suppressed", { chars: clean.length,
                thinking: root._thinking, spoke: root._spokeThisTurn, gateOpen: root._fillerGateOpen })
            return
        }
        root._quickFillerText = ""   // consumed
    ...
    function _cancelSlowOpWatch() {
        slowOpTimer.stop()
        fillerGateTimer.stop()           // [barista-fork] the answer won the race → the held filler is moot
        root._quickFillerText = ""       // [barista-fork] never let a held filler speak into a resolved turn
        root._thinkingCue = false
    ...
    function _askWithContext(dataBlock, unused) {
        ...
        _mods.coaching = "DIALING FRAMEWORK — suggest ONE concrete change for the next shot when it helps. Read taste on two axes: "
            ...
            + "→ coarser or a shorter ratio; nudge dose for strength) — but they are often NOT the right lever: a harsh "
            + "FINISH, a muting pressure SPIKE, thin BODY, or channeling that survives good prep are PROFILE / pre-infusion "
            + "problems grinding can't fix (a declining tail for a harsh finish; a longer, softer pre-infusion to saturate "
            + "and even the puck; a flow cap to kill a spike). recommend_next_shot now reasons across ALL of these — prefer "
            + "it over reaching for grind from memory. GUARD-RAIL: finer does "
            + "NOT always extract more — if a shot CHANNELS or CHOKES at a fine grind, going finer extracts LESS and less "
            + "evenly, so go COARSER (or fix the puck / soften the pre-infusion), never chase it finer. For a pure balance "
            + "fix, settle grind and ratio before temperature or profile; but when the goal is BODY, CLARITY, or a "
            + "harsh/uneven shot, a profile or pre-infusion move may be the RIGHT first change — and if the profile was "
        ...
        persona += "JUST-PULLED SHOT: if sessionContext.justPulledShot is present, a shot finished a few minutes ago and "
            ...
            + "about to save/apply/switch, don't say 'let me save that…' BEFORE it and 'saved!' AFTER; skip the "
            + "pre-announcement and give just the result once. Narrating it beforehand and confirming it after is the "
            + "same thing said twice, and the user hears both. Use their name rarely. At most ONE question per "
        ...
        // [barista-fork] COFFEE BRAIN — the grounded coffee-science tools (translate_taste / recommend_next_shot /
        // plan_for_goal), backed by a peer-reviewed + named-authority knowledge base. They are the REASONING
        ...
        _mods.coffeebrain = "COFFEE BRAIN (grounded reasoning tools) — you have a coffee-science knowledge base built from "
            ...
            + "AUTOMATICALLY reads their most recent shot off the machine — the real dose, yield, ratio, grind, "
            + "duration, and the OBJECTIVE trace (whether it actually channeled or ran fast/slow, from the shot's own "
            + "detectors) — plus the bean's prior-shot history, and returns those to you as `machine_data` and "
            + "`history`. So you usually just pass the taste word and let the tool ground the rest; reason and speak "
            + "from what it hands back (that's the real shot, not a guess). It returns the SINGLE best next change — which "
            + "may be a grind, dose, ratio, temperature, OR a profile / pre-infusion move, not just grind — "
            + "already shaped as a checkable hypothesis — speak that: the one change, what should shift, and how "
            + "you'll know if it's wrong ('that one ran a touch fast and came out sour — let's grind two finer; it "
            + "should slow 3-5s and sweeten, and if it turns dry we went too far'). To reason about an older shot "
            + "pass its shot_id. Still propose, don't apply, until they approve.\n"
            + "  • RESPECT the result's flags. prep_gate=true means the shot CHANNELED — fix the puck (WDT/level/tamp) "
            + "and re-pull FIRST; do not chase a grind number over an uneven puck. confidence 'low' or a "
            + "need_more_context list means you're guessing — ask the ONE most useful thing it names (usually the "
            + "trace or the roast) instead of bluffing a fix.\n"
            + "  • When the user states a GOAL in their own words ('sweeter', 'less sharp', 'more chocolatey', "
            + "'punchier', 'a bigger drink', 'more like the café's'), call plan_for_goal and pass the roast — the same "
            + "goal can mean OPPOSITE moves on a light vs a dark roast, and the tool returns the roast-specific path.\n"
            + "  • Use translate_taste when a taste word is unusual or you want to be sure what it points to before you "
            + "advise.\n"
            + "  • MAKING A PROFILE (the pressure/flow curve). When the change you recommend is a PROFILE move — a "
            + "longer/softer pre-infusion, a deeper declining tail, or capping a pressure spike — you can carry it out "
            + "with create_related_profile, the SAME propose-then-approve way you apply a grind or ratio. NEVER edit a "
            + "profile the user didn't make: the default (mode 'copy') makes a NEW profile from the base and leaves the "
            + "original untouched. Before you call it, get their go-ahead, THEN ask two things: (1) what to NAME the new "
            + "profile, and (2) whether it should be a sub-profile of an existing base — default is their current "
            + "active profile. Only offer mode 'edit_in_place' if the base is one the USER created AND they confirm; "
            + "for anything built-in or downloaded the app copies instead and the result tells you so. Pass adjustment "
            + "(preinfusion / declining_tail / cap_spike), base, name, and a short reason. The result says exactly what "
            + "changed and whether it made a copy — confirm that back naturally ('Made you \"Morning Zoka\" off the LRv3 "
            + "with a longer pre-infusion — it's loaded for the next shot'). If it couldn't (e.g. no decline phase to "
            + "deepen), say so plainly and offer another lever. It also activates the result for the next shot.\n"
            + "  • BEYOND the on-device history, you can reach the user's coffee data in the cloud (when internet is "
            + "on): get_visualizer_shot pulls ANY Visualizer shot by its id or share link — the user's own OR "
            + "someone's public shot — so if they paste a visualizer.coffee link or say 'compare mine to this one', "
            + "pull it in and reason over it. search_visualizer_shots lists their own uploaded shots (or the public "
            + "feed) as ids to pull. look_up_bean fetches a bean's origin, process, roast, variety and the roaster's "
            + "tasting notes — reach for it when the bean's details would sharpen your advice or the user asks about "
            + "it. Treat all of this as fair game; use whatever is relevant.\n"
            + "  • CLOSE THE LOOP (learn from what happened). A recommendation is a hypothesis. When a change you "
            + "suggested was applied and the next shot is tasted, note out loud — briefly — whether it moved the way "
            + "you predicted ('there it is, that finer grind sweetened it right up' / 'still sharp — that didn't do it, "
            + "let's try a touch more'), and log that taste so it's on the record. Next time you advise on THIS "
            + "grinder + bean, build on that outcome — the history in the data block is your memory of what has and "
            + "hasn't worked, and grind numbers are specific to this grinder + bean, so learn each pairing rather than "
            + "carrying a number across. Feed what you learned back in: when you already know the last change and how "
            + "it turned out, say so in your recommend_next_shot reasoning.\n"
            + "  • The tools return citations and trust tiers — those are for YOUR confidence, NOT to read aloud. Never "
            + "recite a paper, author, DOI, or the word 'tier'. Speak like a friend who happens to know the science: "
            + "the plain recommendation, warmly, one change at a time. If the user explicitly asks WHY or for the "
        ...
        if (level === "off")
            ...
            persona += "\nBe proactive, but raise the SINGLE most useful thing — do NOT list multiple issues. "
                ...
                + "(shared roast/origin/process). Lean on it when this bean is NEW or thin on history: when its "
                + "currentBeanIsNew flag is set they have NEVER pulled this bean, so this is your best starting "
                + "read ('you haven't dialed this one yet, but your other washed Ethiopians landed around a "
                + "finer grind — want to start there?'); with some history it complements the palate. A "
                + "borrowed grind is a STARTING hint, never a target, since grind is grinder+bean specific. "
                + "When similarBeanExperience is ABSENT (nothing rated resembles this bean) OR the user wants "
                + "to go deeper, reach for the COMMUNITY tools — look_up_bean for the "
        ...
        if (_supportsWebSearch && webOn)
            _mods.web = "You have live web search. AUTOMATICALLY use it for any real-time or factual question "
                ...
                + "Only fall back to web search for weather/stock/news if the fast tool returns an error. "
                + "CRITICAL — NEVER FROM MEMORY: a stock price, weather condition, or news headline you 'remember' "
                + "is ALWAYS stale (your training data is months or years old), so you must NEVER state one from "
                + "memory. ALWAYS call the tool first and answer ONLY from its result. If the tool errors and web "
                + "search can't get it either, say you couldn't get the current figure right now — NEVER recite a "
                + "remembered/old number as if it were current. BE "
            ...
            _mods.web = "For three common real-time questions you have FAST, dedicated tools — use them when the "
                ...
                + "(pass a topic, or a city for local news; omit both for home-location local news). NEVER FROM "
                + "MEMORY: a stock price, weather, or headline you 'remember' is always stale — ALWAYS call the "
                + "tool and answer only from its result; if it errors, say you couldn't get the current value, "
                + "never recite an old one. Do NOT volunteer "
    ...
    Connections {
        ...
        function onResponseReceived(response) {
            ...
            if (root._quickFillerSpoke && root._voice && root._voice.speaking && !root._voice.audible) {
                root._voice.stop()                 // clears pendingSynth + speaking synchronously (see stop())
                root._quickFillerSpoke = false
                root._leadinSpokenText = ""         // never heard → must not dup-suppress the answer
                root._pendingDisplayText = ""
                root._diag("quick_filler_aborted_answer_ready", {})
            }
    ...
    Connections {
        ...
        function onQuickFillerReady(text) {
            ...
            root._trySpeakQuickFiller()   // no-op until _fillerGateOpen; then speaks if still needed
        }
    }
