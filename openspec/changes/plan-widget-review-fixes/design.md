...
**D10 — One plan widget, not three.** The page-aware `plan` and display-only `steamPlan` widget types added by this PR are removed; `ShotPlanItem` absorbs the page-aware behavior behind a `shotPlanShowSteamPlan` toggle (default ON). Rationale: the two extra types doubled registration surface (4 places × 2 + center zone + options gate + tests) and split one user concept ("what will the machine do next") across three palette entries, while the toggle costs one switch row in an existing dialog. Default ON means every placed Shot Plan widget gains steam awareness silently — the desired outcome — and the toggle is the escape hatch. `SteamPlanText` remains as the internal renderer; `Theme.currentOperationMode` remains its context feed. Saved layouts referencing the removed types render nothing (LayoutItemDelegate's default → null) — acceptable, the types never shipped in a release.
