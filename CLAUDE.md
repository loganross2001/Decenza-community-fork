...
- **A comment earns its place by saying what the code cannot. Never narrate what a clean read already shows.** The test is additive: would a competent developer reading this code still need this sentence? If not, delete it. What survives that test is the non-obvious — why a defaulted parameter is forbidden, which of two plausible orderings the platform actually uses, what a measured number was and on what hardware, a citation with file and line, an API that documents the opposite of what its name implies. What does not survive: restating the next line in prose, a narrative of how the code came to be written, a record of drafts and corrections, or the same fact said three ways.
...
- **Centralize anything produced at more than one site — never hand-roll it per call, never copy a helper's body.** A repeated format, prefix, tag, wording, or policy is a drift opportunity: each copy is free to change alone, silently, and nothing fails when one does. Put it behind one function or macro and call that. This is not a tidiness preference, it is how the copies stay true to each other.
  ...
  - **Anything sent to an AI provider or an MCP client gets ONE format, built by ONE assembler.** Both surfaces talk to the same model, and in Decenza they already share a system prompt: `AIConversation::multiShotSystemPrompt` and `ai_advisor_invoke` both call `ShotSummarizer::shotAnalysisSystemPrompt`, ~45,000 characters that name the structured blocks 35 times (`currentBean` 16, `dialInSessions` 7, `bestRecentShot` 5, …) and tell the model to read them by field path. A second renderer therefore does not just risk drift — it breaks a contract the shared prompt already made. It happened: the in-app advisor hand-rendered a prose `### Setup:` header from its own list of setup fields while the MCP payload built the same identity from `ShotIdentity::fields()`, so the basket and puck prep reached one surface from a single table row and never reached the other at all. The prose copy also had to be parsed back out — `getConversationText` recovered the user's question by searching for `"Here's my latest shot:"` and guessing whether the trailing text "looks like a question" — ~70 lines that exist only because the question was glued to a prose payload. Assemble the payload once, in the shared builder, and let both surfaces send it.
  - **When the same data goes out in different increments, the increment is a SELECTION over one definition — never a second list.** A payload legitimately slices its data: the advisor sends the identity three ways, as `currentBean` (the setup now), `dialInSessions[].context` (what a past session shared) and per-shot overrides (what one shot differed on). That is not duplication. Redeclaring the fields per slice is. Measured here: `ShotIdentity`'s 12 `QString` fields are a **strict subset** of `CurrentBeanBlockInputs`'s 16 — every one of them declared twice — and only the hoist path reads the `ShotIdentity::fields()` table, so a new component lands in two of the three slices and silently misses the third. Give the shared fields one definition, let each increment compose it plus its own extras, and put the slice in a helper rather than in each caller's hand-written list.
...
- **Adding a log line? Read `docs/CLAUDE_MD/LOGGING.md` first — it is a gate, not advice.** `scripts/check_log_markers.py` checks first-party runtime C++/headers, QML/JS and native bridges for registered logging helpers, including mixed-owner files and lowercase/dynamic bypasses. Three things that keep going wrong and are all in that doc:
  ...
  - **Ownership follows the operation.** Mixed files use registered helpers per event. Shared formatter and crash-safe exceptions require a concrete call-site reason; the Runtime fallback does not waive source coverage. The covered paths live in the script.
...
- **Before every requested merge, check the PR's OpenSpec change.** Verify implementation and validation readiness, reconcile outstanding tasks honestly, and ensure the change has been archived and the archive committed. Use the CLI command `openspec archive <change-name> --yes`, which also updates the main specs; prefer it over the archive skill. Whenever possible, make the archive the PR's final commit, then push and read the checks for that commit before merging. If the user explicitly holds validation or other work, record the hold and its follow-up without marking it passed. A PR without an associated OpenSpec change does not need an artificial one.
