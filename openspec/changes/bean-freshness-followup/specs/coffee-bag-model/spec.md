...
`storageHint` and `openedDate` are local-only fields, same as `frozenDate`/`defrostDate` — neither is included in `touchesVisualizerFields()` and neither is pushed to or read from Visualizer.
