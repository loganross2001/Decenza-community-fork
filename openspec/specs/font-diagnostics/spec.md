...
Make font resolution answerable from a submitted debug log. Text layout problems are reported by
users on machines the maintainers cannot reach, and the visible symptom — overflow, a substituted
glyph — does not say which font actually rendered it. These diagnostics record what resolved and
how it measured, so a remote report can be attributed to a specific font rather than guessed at.
