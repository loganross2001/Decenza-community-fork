# refractometer-review-page-discovery Specification (delta)
...
Continuous scanning SHALL end when the post-shot review page deactivates (another page is pushed on top or the page is popped) or is destroyed. Off the review page, refractometer discovery behavior SHALL be unchanged: the periodic background reconnect cadence applies.

#### Scenario: Leaving the review page ends the hunt

- **WHEN** the hunt is active and the user navigates away from the post-shot review page
- **THEN** the in-progress scan (if any) completes naturally and is not restarted by the hunt
- **AND** subsequent refractometer discovery follows the existing periodic reconnect cadence
...
- **THEN** the existing reconnect retry schedules the first scan (approximately 5 seconds later)
