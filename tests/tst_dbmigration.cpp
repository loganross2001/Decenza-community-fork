...
private slots:
    ...
    void freshDbCreatesSchema() {
        ...
        withRawDb(path, "fresh_schema", [](QSqlDatabase& db) {
            ...
            QCOMPARE(getSchemaVersion(db), 29);
        });
    }
    ...
    void v16_columnIsDropped()
    {
        ...
        QCOMPARE(versionFound, 30);  // latest after full chain (recipes bag_id, +1 for barista vsp mig 25)
    ...
    void v22_reinitDoesNotDuplicatePackages() {
        ...
        { ShotHistoryStorage s; initAndClose(path, s); }
        withRawDb(path, "v22_ver", [](QSqlDatabase& db) { QCOMPARE(getSchemaVersion(db), 29); });
