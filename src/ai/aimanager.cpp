...
void AIManager::requestCoachPhrasebook(const QString& requestToken, const QString& contextBlock)
{
    ...
    const QString systemPrompt = QStringLiteral(
        ...
        "numbers unless natural. Professional and concrete — a barista instructor in plain technical terms "
        "(name the steam wand, the tip, the milk, the pitcher, the puck, the flow, the pressure, the shot), "
        "warm but unambiguous; NEVER vague or double-meaning ('almost there', 'a little longer', 'keep going', "
        "'nice and slow', 'hold it', 'go deeper', 'get ready' without saying get ready TO WHAT). "
        "The ids fall into TWO KINDS, and they must sound different: "
        "STEAM cues (steam-stretch, steam-roll, steam-almost, steam-done) are LIVE-ACTIONABLE — the user is "
        "steaming NOW and can act this second — so say WHAT to do to WHICH part (e.g. 'ease the wand deeper to "
        "spin the milk', not 'go deeper'; 'steam's nearly done, ready to shut it off', not 'almost there'). "
        "EXTRACTION cues (no-puck, channeling, flow-fast, flow-slow) are OBSERVE-ONLY — the shot is already "
        "pouring and the grind and dose CANNOT be changed mid-pour. Just narrate what is happening in THIS "
        "pour right now — the pace, the flow, the pressure, whether it's tracking to target — like a coach "
        "watching alongside. NEVER prescribe a fix and NEVER mention the next shot, grinding, the dose, or "
        "dialing in; that is the after-shot debrief's job, not a live cue (e.g. 'flow's picking up, this'll "
        "come in bright', not 'grind finer next time'; 'running slow, the puck's tight today', not 'go coarser "
        "next shot'). "
...
void AIManager::analyzeConversation(const QString& systemPrompt, const QJsonArray& messages,
                                    bool webSearch, bool clientTools, bool streaming, qint64 shotId)
{
    ...
    // [barista-fork] logPrompt uses the diagnostic operation's resolved provider when one is dispatched
    // (upstream diagnostics-correlation), falling back to selectedProvider() otherwise.
    logPrompt(m_logOperation ? m_logOperation->provider : selectedProvider(), systemPrompt, m_lastUserPrompt);
