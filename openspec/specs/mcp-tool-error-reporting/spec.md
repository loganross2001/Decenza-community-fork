...
## Purpose
How an MCP tool signals that it failed, and how that failure is represented on
the wire: a tool returns an `error` key, the server marks the tool call failed
with `isError: true` while leaving the error text readable in the response
content, and a failed tool call remains a JSON-RPC `result` — JSON-RPC `error` is
reserved for protocol faults. One shared path decides what a failed tool call
looks like, so no tool and no call site hand-rolls the marking.
## Requirements
