...
| `src/ai/aimanager.cpp` | Barista conversation entry + bag extraction + the positional `RequestOptions{...}` init | **Union.** When upstream touches AIManager, keep the barista turn wiring; the positional `RequestOptions{webSearch, clientTools, timeoutMs, forceRespond, imageData, imageMediaType}` init must list every field (C++17, no designated initializers) — add new fields there when the struct grows. |
