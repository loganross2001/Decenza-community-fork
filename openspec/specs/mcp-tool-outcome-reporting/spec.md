# mcp-tool-outcome-reporting Specification

## Purpose
What a tool's result must reflect about the operation it performed. `success`
means the operation happened, not that the tool was called: a tool consults the
operation's own outcome rather than assuming it, an asynchronous tool answers on
every terminal signal, an unavailable dependency is reported as an error rather
than as emptiness, dropped inputs are named, a no-op is a success flagged as a
no-op, and an unsupported device capability is refused rather than faked.

The companion to [`mcp-tool-error-reporting`](../mcp-tool-error-reporting/spec.md),
and the half that one cannot reach: marking is driven off a tool's `error` key,
so a tool that never writes one ships as a successful call however little it
did.
## Requirements
