...
- **No list of bundled profiles**: CMake globs `resources/profiles/*.json` at configure time (`CONFIGURE_DEPENDS`, so a new file is picked up by the next build) and writes the `.qrc` into the build directory for the app and the test targets alike. Drop a JSON into the directory and it ships. Two hand-kept lists used to exist — `resources.qrc` for the app, `profiles.qrc` for the tests — and drifted for two months: nine profiles (Adaptive v3 among them) reached every test binary and no release. `scripts/check_profile_resources.py` fails a PR if any `.qrc` in the repo lists a profile by hand. When de1app replaces a built-in in place under a new title (best_practice.tcl, Adaptive v2 → v3), retire the old file and add its successor to `kSuccessor` in `ProfileManager::refreshProfiles()` so favorites, current profile and auto-load follow it.
...
**Eligibility**: only favorites can be the auto-load — the removed Selected list was folded into favorites by a one-time upgrade (see "Favorites order" below). If the pinned profile is deleted or un-favorited, the setting is cleared eagerly (in `SettingsApp::removeFavoriteProfile` / `ProfileManager::deleteProfile`) and at trigger time (`ProfileManager::loadAutoLoadProfileIfNeeded()`), with `autoLoadStaleCleared` emitted so the UI can toast.
...
- A `Set Auto-Load` / `Disable Auto-Load` entry in the card's ⋮ actions dialog, visible only when the card's profile is a favorite.
