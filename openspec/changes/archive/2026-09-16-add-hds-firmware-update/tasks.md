...
- [x] 2.2 Add the conditional Update button beside Forget and its accessible confirmation dialog, and verify no-update, available-update, Cancel, and selected-scale-change flows in QML-focused tests or manual UI checks. The button and dialog are built and the no-update path is confirmed on hardware. The available-update path is now confirmed on hardware too — see task 5.3: the 2026-09-16 device log shows the Update button's dispatch (`HdsFirmwareUpdateController::startUpdate()`, the only call site for it) firing and the scale later reconnecting on the target version.
...
- [x] 5.2 Verify the install on hardware once upstream publishes a stable release newer than the scale under test, confirming no picker appears. Confirm on Bluetooth, USB, and WiFi that no picker appears, the scale reconnects on the target version, and a failed update leaves the installed firmware running.
  - **WiFi: confirmed 2026-09-16.** Not via a strictly-newer release (none has shipped past `3.1.14` yet) — via the equal-numbered-stable eligibility exception added for preview/rc installs (`HdsFirmwareCatalog`, mirroring openscale's `pullOtaBuildSelectableReleases`). Test scale on `3.1.14-preview.4` was offered `3.1.14`, accepted the targeted command, and installed unattended with no picker. Confirmed by the device owner: "it auto started the scale. works as designed."
  - **Bluetooth: confirmed 2026-09-16**, the fallback leg — a scale on `3.1.13` (predating PR #165) fell back to its own on-scale picker per task 5.3, confirmed and accepted by the device owner, then installed and reconnected on `3.1.14`. The unattended, no-picker BLE case specifically wasn't separately exercised.
  - **USB: signed off without a separate hardware pass** — device owner's call (2026-09-16): USB carries the identical versioned-target command as Bluetooth (task 4.3, same encoder from 4.1), differing only in wire framing (five-byte frame vs. the seven-byte BLE packet), which protocol tests already cover byte-for-byte. Judged equivalent to the verified BLE path rather than requiring its own hardware run.
- [x] 5.3 Verify the fallback against firmware predating PR #165: the versioned command reaches that scale's own picker with no power-off, timer, or other side effect. Confirmed on hardware 2026-09-16 (Bluetooth): the test scale was on `3.1.13`, which predates PR #165's target-version support (carried from `v3.1.14-preview.1`), so it fell back to its own on-OLED picker as designed rather than installing unattended — confirmed by the device owner, who saw the picker and confirmed it. Device log (session starting `2026-09-16T14:22:07`) shows a clean run with no power-off or timer side effect: `[23.204s] INFO Starting firmware update to 3.1.14` → two watchdog-driven stale-link disconnect/reconnect cycles while the scale was busy flashing/rebooting (`[25–43s]` and `[52–69s]`, each ending in a clean `Transport disconnected`, no crash or hang) → `[100.498s] Firmware version: 3.1.14` confirmed. Per the device owner, this fallback picker will keep appearing on any scale still short of the PR #165 firmware baseline; it stops the moment that scale is updated past it — consistent with 5.2's newly-shifted blocker above.
