# background-color-presets Specification

## Purpose

Defines the curated catalogue of built-in background colours and the independent catalogue of
patterns drawn over them, how a selection persists (`Settings.theme.backgroundPreset`), and how
the readable foreground — text, icons, borders, card/dialog/inset fills — is derived from the
chosen colour rather than taken from the theme, so a preset stays legible under any theme and in
either light or dark mode. Also covers how the semantic palette is nudged to clear a contrast
floor against the page, the mutual exclusivity of a preset and a background image, the cases in
which a later explicit background choice clears the preset, and the sectioned background chooser
that presents all of it.
## Requirements
